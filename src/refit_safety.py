"""
refit_safety.py — Re-fit SafetyClassifier with updated thresholds, then evaluate.

Run this after editing safety.py thresholds, without re-training the trajectory model.
"""
...
def main():
    ...
    model.eval()
    print(f"Trajectory model loaded  |  device={device}")

    # ── re-fit safety classifier with updated thresholds ─────────────────────
    print("\nRe-fitting SafetyClassifier on Y_train (original meter scale)...")
    clf = SafetyClassifier()
    clf.fit(Y_train)
    clf.save("outputs/checkpoints/safety_clf.pkl")

    # ── safety classification reports ─────────────────────────────────────────
    val_report  = None
    test_report = None
    ...
    # ── trajectory metrics (for completeness) ─────────────────────────────────
    ...
    print("\nDone. Updated outputs/results.json")
